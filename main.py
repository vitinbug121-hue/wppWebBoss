...
class AppColetorPro:
    ...
    def setup_ui(self):
        ...
        tk.Button(btn_frame, text="ATUALIZAR BLT PAGOS", command=self.atualizar_blt_pagos, bg="#189108", fg="white", **estilo).grid(row=0, column=4, padx=5)
        ...
        try:
            ...
            for i, pedido in enumerate(pedidos):
                       ...
                       # diferenca = datetime.now() - data_envio
                        if env_boleto and boletos_contagem < lim_boleto:
                            ...
                            if envio.status_code in [200, 201]:
                                dados_pedido['boleto_enviado'] = True
                                dados_pedido['data_boleto'] = datetime.now().strftime("%d/%m/%Y %H:%M")
                                dados_pedido['codigo_boleto'] = boleto_numero
                                dados_pedido['id_payment'] = str(row['id_payment'])
                ...
                if order_id in db and (dados_pedido.get('solicitado') and dados_pedido.get('data_solicitacao_inicial')):
                    ...
                    try:
                        ...
                        if env_rastreio and rastreios_contagem < lim_rastreio and not dados_pedido.get('rastreio_enviado'): 
                            ...
                            dados_pedido['rastreio_enviado'] = True
                            dados_pedido['data_rastreio'] = datetime.now().strftime("%d/%m/%Y %H:%M")
                            dados_pedido['codigo_rastreio'] = cod_rastreio
                            boletos_contagem += 1
                            self.salvar_db(db)
                            continue
                    except Exception as e:
                ...
                if order_id in db and (dados_pedido.get('solicitado') and not dados_pedido.get('numero_extraido')):
                    for m in conversaCompleta:
                        ...
                        if match:
                                ...
                                dados_pedido['zap_extraido'] = zap
                                dados_pedido['numero_extraido'] = True
                    ...
                    if data_solicitacao:
                        try:
                            ...
                            if diferenca.days >= 1 and not dados_pedido.get('numero_extraido') and not dados_pedido.get('tentativa_', 0) < 2:
                                ...
                                #envio = requests.post(url_msg, json=payloadReenvio, headers=headers)
                                if envio.status_code in [200, 201]:
                                    dados_pedido['data_solicitacao'] = datetime.now().strftime("%d/%m/%Y %H:%M")
                                    dados_pedido['tentativa_'] = dados_pedido.get('tentativa_', 0) + 1
                ...
                if ja_enviado_no_ml:
                    ...
                    dados_pedido['solicitado'] = True
                    dados_pedido['buyer_id'] = pedido.get('buyer', {}).get('id')
                    dados_pedido['pack_id'] = pedido.get('pack_id')
                    dados_pedido['corProduto'] = corProduto
                    dados_pedido['produto'] = nome_prod
                    dados_pedido['valor'] = valor
                    dados_pedido['data_solicitacao'] = datetime.now().strftime("%d/%m/%Y %H:%M")
                    dados_pedido['data_solicitacao_inicial'] = datetime.now().strftime("%d/%m/%Y %H:%M")
                    continue
